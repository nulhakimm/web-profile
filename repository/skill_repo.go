...
func (repo *SkillRepoImpl) Find(ctx context.Context, client *firestore.Client) ([]*domain.Skill, error) {
...
func (repo *SkillRepoImpl) Save(ctx context.Context, client *firestore.Client, skill *domain.Skill) error {

	_, _, err := client.Collection("skills").Add(ctx, skill)
	if err != nil {
		return fmt.Errorf("failed to create new skill : %v", err)
	}
